...
class Number extends g.E {
  ...
  constructor(param) {
    ...
    this.fSprite = new Array(this.maxDigit);
    // 
    this.createEntity();
  }
  //
  ...
  setNumber(n) {
    ...
    for (let i = 0; i < this.maxDigit; i++) {
      if (this.align === "left") {
        ...
        this.fSprite[i].x = this.pitch * (digit - i) + this.pitch / 2;
      }
   ...
   */
  change() {
    const diff = this.nextNum - this.showNum;
    if (diff <= 0) return;
    const add = Math.max(Math.floor(diff / this.changeFrameCnt), 1);
    this.changeFrameCnt--;
